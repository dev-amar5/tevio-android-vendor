...
class CircularProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        progressBarWidth = attributes.getDimension(
            R.styleable.CircularProgressBar_cpb_progressbar_width, progressBarWidth
        ).pxToDp()
        ...
        progressBarColor = attributes.getInt(
            R.styleable.CircularProgressBar_cpb_progressbar_color, progressBarColor
        )
        ...
        attributes.getColor(
            R.styleable.CircularProgressBar_cpb_background_progressbar_color_start, 0
        ...
        progressDirection = attributes.getInteger(
            R.styleable.CircularProgressBar_cpb_progress_direction, progressDirection.value
        ).toProgressDirection()
        ...
        indeterminateMode = attributes.getBoolean(
            R.styleable.CircularProgressBar_cpb_indeterminate_mode, indeterminateMode
        )
    ...
    private fun createLinearGradient(
        startColor: Int, endColor: Int, gradientDirection: GradientDirection
    ): LinearGradient {
        var x0 = 0f
        var y0 = 0f
        var x1 = 0f
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        rectF.set(
            /* left = */ 0 + highStroke / 2, /* top = */
            0 + highStroke / 2, /* right = */
            min - highStroke / 2, /* bottom = */
            min - highStroke / 2
        )
    }
    //endregion
    ...
    fun setProgressWithAnimation(
        ...
        progressAnimator = ValueAnimator.ofFloat(
            if (indeterminateMode) progressIndeterminateMode else this.progress, progress
        )
            ...
            (animation.animatedValue as? Float)?.also { value ->
                ...
                if (indeterminateMode) {
                    ...
                    startAngleIndeterminateMode =
                        DEFAULT_START_ANGLE + if (progressDirectionIndeterminateMode.isToRight()) updateAngle else -updateAngle
                }
            }
        }
