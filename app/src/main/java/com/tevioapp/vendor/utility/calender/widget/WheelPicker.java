package com.tevioapp.vendor.utility.calender.widget;
...
import com.tevioapp.vendor.utility.calender.LocaleHelper;
...
public abstract class WheelPicker<V> extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + selectedItemPosition, drawnOffsetPos = -mHalfDrawnItemCount; drawnDataPos < drawnDataStartPos + selectedItemPosition + mDrawnItemCount; drawnDataPos++, drawnOffsetPos++) {
            String data = "";
            if (isCyclic) {
        ...
        if (isEnabled()) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (Math.abs(downPointY - event.getY()) < touchSlop && computeDistanceToEndPoint(scroller.getFinalY() % mItemHeight) > 0) {
                        isClick = true;
                        break;
                    }
                    isClick = false;
    ...
    public void setMaximumWidthTextPosition(int position) {
        if (!isPosInRang(position)) {
            throw new ArrayIndexOutOfBoundsException("Maximum width text Position must in [0, " + adapter.getItemCount() + "), but current is " + position);
        }
